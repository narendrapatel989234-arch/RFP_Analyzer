const fs = require('fs');
let content = fs.readFileSync('src/app/rfp-details/page.tsx', 'utf-8');

const interfaces_and_dropzone = 
interface TableRow {
  id: string;
  name: string;
  description: string;
...
const state_regex = /const \[capabilityDocs, setCapabilityDocs\] = useState<File\[\]>\(\[\]\)[\s\S]*?const \[partnerCapabilitiesDocs, setPartnerCapabilitiesDocs\] = useState<File\[\]>\(\[\]\)/;
const new_state = 
...
.trim();
content = content.replace(state_regex, new_state);

const clear_regex = /const handleClearAll = \(\) => \{[\s\S]*?setPartnerCapabilitiesDocs\(\[\]\)/;
const new_clear = 
...
.trim();
content = content.replace(clear_regex, new_clear);

const log_regex = /capabilityDocs,[\s\S]*?supportingDocs,/;
const new_log = "supportingDocs,";
content = content.replace(log_regex, new_log);

const jsx_regex = /\{\/\* Section 3 \- Upload Documents \*\/\}([\s\S]*?)<\/section>/;
const new_jsx = 
...
.trim();
content = content.replace(jsx_regex, new_jsx);
...
;
content = content.replace('      </main>', modal);

fs.writeFileSync('src/app/rfp-details/page.tsx', content, 'utf-8');
console.log("Done");
